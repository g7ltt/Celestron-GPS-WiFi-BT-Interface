 ...
 *
 * Version 3.8
 *  -- Add 'exit' command for port 3000.
 *  -- Fix Bluetooth:  replace BT_AUX_ENABLED with BT_ENABLED.
 * Version 3.7
...
#define BT_ENABLED        true    // When false, Bluetooth code is completely left out
...
#define GPS_TX_PIN                  22  // TX-to-GPS pin. Possibly wired to the GPS RX pin, but we never send anything (CONFLICT with I2C SCL)
 ...
 */
static bool p3000_decoder (char b, void (*p3000_write)(const char *))
{
...
static void service_w3000()
{
  ...
  if (wifi_port_is_connected(w3000, &connected, poll_for_new_w3000_connection, "w3000")) {
    while (w3000.available()) {
      if (p3000_decoder(w3000.read(), w3000_write))
        w3000.stop();
    }
    w3000.flush();
  }
